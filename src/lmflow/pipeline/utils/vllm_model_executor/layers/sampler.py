...
from lmflow.pipeline.utils.vllm_model_executor.parallel_utils.communication_op import (
    tensor_model_parallel_all_gather)
from lmflow.args import InferencerArguments
from lmflow.args import SamplingType
# from vllm.inferencer_args import SamplingParams
from lmflow.pipeline.utils.vllm_sequence import (PromptLogprobs, SampleLogprobs, SamplerOutput,
...
def _get_penalties(
    ...
    for i, seq_group in enumerate(input_metadata.seq_groups):
        seq_ids, inferencer_args = seq_group
        p = inferencer_args.presence_penalty
        f = inferencer_args.frequency_penalty
        r = inferencer_args.repetition_penalty
        if (i < input_metadata.num_prompts
                and inferencer_args.prompt_logprobs is not None):
...
def _apply_logits_processors(logits: torch.Tensor,
    ...
    for seq_ids, inferencer_args in input_metadata.seq_groups:
        logits_processors = inferencer_args.logits_processors
...
def _get_top_p_top_k(
    ...
    for i, seq_group in enumerate(input_metadata.seq_groups):
        ...
        top_k = min(inferencer_args.top_k, vocab_size)
...
def _greedy_sample(
    selected_seq_groups: List[Tuple[List[int], InferencerArguments]],
...
def _random_sample(
    ...
    for seq_group, is_prompt in zip(selected_seq_groups, is_prompts):
        if is_prompt:
            seq_ids, inferencer_args = seq_group
            max_best_of = max(max_best_of, inferencer_args.best_of)
    ...
    for seq_group, is_prompt in zip(selected_seq_groups, is_prompts):
        ...
        if is_prompt:
            ...
            parent_ids = [0] * inferencer_args.best_of
            next_token_ids = random_samples[
                sample_idx, :inferencer_args.best_of].tolist()
        else:
            # Generation phase.
            parent_ids = list(range(num_parent_seqs))
...
def _beam_search_sample(
    ...
    for seq_group, is_prompt in zip(selected_seq_groups, is_prompts):
        seq_ids, inferencer_args = seq_group
        num_parent_seqs = len(seq_ids)
        beam_width = inferencer_args.num_beams
...
def _sample(
    ...
    for i, seq_group in enumerate(input_metadata.seq_groups):
        _, inferencer_args = seq_group
        sampling_type = inferencer_args.sampling_type
...
def _get_logprobs(
    ...
    for i, (seq_group, sample_result) in enumerate(
        ...
        if (i < input_metadata.num_prompts
            ...
            largest_num_logprobs = max(largest_num_logprobs,
                                       inferencer_args.prompt_logprobs)
            prompt_len = input_metadata.prompt_lens[i]
        ...
        if inferencer_args.logprobs is not None:
            largest_num_logprobs = max(largest_num_logprobs,
                                       inferencer_args.logprobs)
        sample_idx += num_parent_seqs
    assert sample_idx == logprobs.size(0)
    ...
    for i, (seq_group, sample_result) in enumerate(
        ...
        if (i < input_metadata.num_prompts
                and inferencer_args.prompt_logprobs is not None):
            num_logprobs = inferencer_args.prompt_logprobs
            prompt_len = input_metadata.prompt_lens[i]
            prompt_tokens = input_metadata.seq_data[
                seq_ids[0]].prompt_token_ids
        ...
        # Sample logprobs
        num_logprobs = inferencer_args.logprobs
